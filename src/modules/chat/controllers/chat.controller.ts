...
export const getChatRooms = async (
  ...
  try {
    ...
    // Check if the user is also a registered worker
    ...
    const formattedRooms = roomsWithoutSelfChat.map((room) => {
      ...
      return {
        ...
        unread: unreadCount,
        avatar,
        avatarBg: isUserSide ? '#00A884' : '#0367da', // Different color depending on if chatting with worker or user
